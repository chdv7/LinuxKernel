## Домашнее задание
Модуль ядра Linux с аллокатором памяти на основе Bitmap

(оптимизация для выделения маленьких блоков)  


Цель:  
реализовать модуль ядра Linux с аллокатором памяти на основе bitmap, обеспечивающий выделение и освобождение памяти с синхронизацией доступа и управлением через модульные параметры.
...
Выделяет несколько блоков разных размеров  
Проверяет статистику  
Освобождает блоки в различном порядке  
Проверяет статистику после освобождения  
